...
- (void)setupView:(CAEAGLLayer *)layer frontImage:(UIImage *)frontImage backImage:(UIImage *)backImage
{	
  ...
  GLuint width = CGImageGetWidth(frontImage.CGImage);
  GLuint height = CGImageGetHeight(frontImage.CGImage);
  ...
  CGContextDrawImage(bitmapContext, CGRectMake( 0, 0, width, height ), frontImage.CGImage );
  ...
  width = CGImageGetWidth(backImage.CGImage);
  height = CGImageGetHeight(backImage.CGImage);
  ...
  CGColorSpaceRelease(colorSpace);
  CGContextClearRect(bitmapContext, CGRectMake( 0, 0, width, height ) );
  CGContextDrawImage(bitmapContext, CGRectMake( 0, 0, width, height ), backImage.CGImage );
...
//  glDrawElements(GL_LINES, numFaces * 3, GL_UNSIGNED_SHORT, frontFaces);
  
  // Draw the back page
  glBindTexture(GL_TEXTURE_2D, texture[1]);
  glDrawElements(GL_TRIANGLES, numFaces * 3, GL_UNSIGNED_SHORT, backFaces);
//  glDrawElements(GL_LINES, numFaces * 3, GL_UNSIGNED_SHORT, backFaces);
#endif
